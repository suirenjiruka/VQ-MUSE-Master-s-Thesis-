...
# Keep Transformers on its PyTorch backend even when an old TensorFlow package
...
class VQMotionTrainer:
    def __init__(self, cfg, vq_model, vqmotion_model, eval_wrapper, device, edit_eval_wrapper=None, resume_ckpt=None):
        #config setup
        self.config = cfg
        self.vq_model = vq_model
        self.vq_model.eval()
        self.training_model = vqmotion_model
        ...
        self.branch_lr_multiplier = float(getattr(cfg.training, "branch_lr_multiplier", 1.0))
        self.best_g2t_r1 = 0.0
        self.best_g2t_avgr = float("inf")
    ...
    def _optimizer_param_groups(self, lr, weight_decay):
        buckets = {
            (branch, decay): []
            for branch in ("main", "source", "delta")
        ...
        for name, param in self.training_model.named_parameters():
            if not param.requires_grad:
                continue
            lname = name.lower()
            if name.startswith("control_encoder."):
                branch = "source"
            elif name.startswith(delta_prefixes):
                branch = "delta"
            else:
                branch = "main"
            use_decay = not (
                param.ndim < 2 or "norm" in lname or "embed" in lname or "embedding" in lname
            )
            buckets[(branch, use_decay)].append(param)

        groups = []
        for branch in ("main", "source", "delta"):
            lr_multiplier = 1.0 if branch == "main" else self.branch_lr_multiplier
            for use_decay in (True, False):
                params = buckets[(branch, use_decay)]
                if not params:
                    continue
                groups.append({
                    "params": params,
                    "lr": float(lr) * lr_multiplier,
    ...
    def _branch_learning_rates(self):
        rates = {}
        for group in self.optimizer.param_groups:
            branch = group.get("branch_name", "main")
            rates.setdefault(branch, float(group["lr"]))
        return rates

    def _best_metrics_state(self):
        return {
            "g2t_r1": float(self.best_g2t_r1),
            "g2t_avgr": float(self.best_g2t_avgr),
            "fid": float(self.best_fid),
        }

    def _restore_best_metrics(self, state):
        if not state:
            return
        self.best_g2t_r1 = float(state.get("g2t_r1", self.best_g2t_r1))
        self.best_g2t_avgr = float(state.get("g2t_avgr", self.best_g2t_avgr))
        self.best_fid = float(state.get("fid", self.best_fid))
    ...
    def _apply_curriculum_stage(self, epoch, train_loader, steps_per_epoch):
        ...
        self.branch_lr_multiplier = float(stage.get(
            "branch_lr_multiplier",
            getattr(self.config.training, "branch_lr_multiplier", 1.0),
        ))
        ...
        if changed and stage.get("reset_optimizer", False) and (self.active_curriculum_stage is not None or direct_resume_entry):
            ...
            print(
                f"reset optimizer/scheduler for {stage_name}: "
                f"base_lr={stage.get('lr', self.config.training.lr)}, "
                f"branch_lr_multiplier={self.branch_lr_multiplier:.2f}, steps={remaining_steps}"
            )
        ...
        if changed:
            lr_text = ", ".join(
                f"{branch}={branch_lr:.3e}"
                for branch, branch_lr in self._branch_learning_rates().items()
            )
            print(f"optimizer learning rates: {lr_text}")
    ...
    def _model_state_dict(self, compact=False):
        model_state_dict = self.training_model.state_dict()
        # The frozen T5 weights are loaded separately and do not belong in checkpoints.
    ...
    def save(self, save_pth, epoch: int, eval_metrics=None, compact=False):
        model_state_dict = self._model_state_dict(compact=compact)

        state = {
            'training_model': model_state_dict, # model parameter
            'epoch': epoch,
            'seed': int(self.config.exp.seed),
            'best_metrics': self._best_metrics_state(),
            'eval_metrics': self._plain_metrics(eval_metrics),
            'curriculum_stage': self.active_curriculum_stage,
            'branch_lr_multiplier': float(self.branch_lr_multiplier),
        }
        if compact:
            state['inference_only'] = True
            state['weights_dtype'] = 'float32'
        else:
    ...
    @staticmethod
    def _unique_checkpoint_path(save_pth):
        if not os.path.exists(save_pth):
            return save_pth
        stem, ext = os.path.splitext(save_pth)
        index = 1
    ...
    def train(self, train_loader, val_loader, eval_loader, gen_eval_loader=None):
        ...
        if self.config.exp.is_continue == True:
            ...
            self._restore_best_metrics(checkpoint.get('best_metrics'))
            self.active_curriculum_stage = checkpoint.get('curriculum_stage')
            self.branch_lr_multiplier = float(checkpoint.get(
                'branch_lr_multiplier', self.branch_lr_multiplier
            ))

            optimizer_loaded = 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint
            if optimizer_loaded:
                try:
                    self.optimizer.load_state_dict(checkpoint['optimizer'])
                    self.lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
                except (KeyError, ValueError):
                    print("optimizer state is incompatible with current parameter groups; reset optimizer")
            ...
            if not optimizer_loaded:
                ...
                if stage is not None:
                    ...
                    resume_lr = stage.get("lr", self.config.training.lr)
                    resume_warmup = stage.get("warm_up_iter", self.config.training.warm_up_iter)
                    self.active_curriculum_stage = stage.get("name", self.active_curriculum_stage)
                else:
                    resume_lr = self.config.training.lr
                    resume_warmup = self.config.training.warm_up_iter
                remaining_steps = max(1, (max_epoch - epoch) * len(train_loader))
                self._reset_optimizer_scheduler(resume_lr, remaining_steps, resume_warmup)
                print(
                    "scheduler state is reset with optimizer: "
                    f"base_lr={resume_lr}, branch_lr_multiplier={self.branch_lr_multiplier:.2f}"
                )
            print(f"resume from completed epoch {checkpoint['epoch']}; next epoch {epoch}")
            print(
                "restored best metrics: "
                f"G2T R@1={self.best_g2t_r1:.5f}, AvgR={self.best_g2t_avgr:.2f}, "
                f"FID={self.best_fid:.5f}"
            )
            if self.use_ema:
        ...
        while epoch < max_epoch:
            ...
            for i, batch in tqdm(enumerate(train_loader)):
                ...
                if current_iter % log_period == 0:
                    ...
                    for branch, branch_lr in self._branch_learning_rates().items():
                        self.logger.add_scalar(f"train/lr_{branch}", branch_lr, current_iter)
                    # print logs
            ...
            if eval_metrics is not None:
                current_g2t_r1 = float(eval_metrics["g2t_r1"])
                current_g2t_avgr = float(eval_metrics["g2t_avgr"])
                current_fid = float(eval_metrics["fid"])
                improve_g2t = current_g2t_r1 > self.best_g2t_r1
                tie_break = (
                    current_g2t_r1 == self.best_g2t_r1
                    and current_g2t_avgr < self.best_g2t_avgr
                )
                improve_fid = current_fid < self.best_fid
                if improve_g2t or tie_break:
                    print(
                        f"G2T improved: R@1 {self.best_g2t_r1:.5f}->{current_g2t_r1:.5f}, "
                        f"AvgR {self.best_g2t_avgr:.2f}->{current_g2t_avgr:.2f}"
                    )
                    self.best_g2t_r1 = current_g2t_r1
                    self.best_g2t_avgr = current_g2t_avgr
                if improve_fid:
                    print(f"FID improved from {self.best_fid:.5f} to {current_fid:.5f}")
                    self.best_fid = current_fid

                # Update all best values before either checkpoint is serialized.
                if improve_g2t or tie_break:
                    self.save(
                        pjoin(self.config.exp.model_dir, 'best.tar'), epoch,
                        eval_metrics=eval_metrics,
                    )
                if improve_fid:
                    self.save(
                        pjoin(self.config.exp.model_dir, 'best_fid.tar'), epoch,
                        eval_metrics=eval_metrics,
                    )

                save_high_g2t = bool(getattr(
                    self.config.training, "save_high_g2t_checkpoints", True
                ))
                high_threshold = float(getattr(
                    self.config.training, "high_g2t_threshold", 0.45
                ))
                if save_high_g2t and current_g2t_r1 > high_threshold:
                    seed = int(self.config.exp.seed)
                    high_name = (
                        f"high_g2t_e{epoch:03d}_s{seed}_r1_{current_g2t_r1:.4f}.tar"
                    )
                    high_path = self._unique_checkpoint_path(
                        pjoin(self.config.exp.model_dir, high_name)
                    )
                    self.save(
                        high_path, epoch, eval_metrics=eval_metrics, compact=True
                    )
                    print(f"preserved high-G2T inference checkpoint: {high_path}")
            ...
            # Save resumable checkpoints after restoring the non-EMA training weights.
            if epoch % 50 == 0:
                self.save(
                    pjoin(self.config.exp.model_dir, f'_{epoch}.tar'), epoch,
                    eval_metrics=eval_metrics,
                )
            self.save(
                pjoin(self.config.exp.model_dir, 'latest.tar'), epoch,
                eval_metrics=eval_metrics,
            )
