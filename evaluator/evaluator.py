...
def evaluation_generation_hml_mixed(eval_loader, trans, vq_model, writer, ep, eval_wrapper, device, time_steps=10, cond_scale=4, source_cond_scale=1.0, text_delta_scale=1.0, unit_length=4, temperature=1,
    ...
    for batch in tqdm(eval_loader):
        ...
        if len(batch) < 12:
            print("Generation evaluator needs word_embeddings/pos_one_hots/sent_len from HML3DMotionEditDataset.")
            return None

        word_embeddings, pos_one_hots, sent_len = batch[9], batch[10], batch[11]
        gen_mask = ~has_source.bool()
        if not gen_mask.any():
            continue

        caption = [cap for cap, keep in zip(caption, gen_mask.tolist()) if keep]
        tgt_motion = tgt_motion[gen_mask].to(device).float()
        m_length = m_length[gen_mask].to(device).long()
        word_embeddings = word_embeddings[gen_mask].to(device).float()
        pos_one_hots = pos_one_hots[gen_mask].to(device).float()
        ...
        word_embeddings = word_embeddings[sort_idx]
        pos_one_hots = pos_one_hots[sort_idx]
        sent_len = sent_len[sort_idx]
        tgt_motion = tgt_motion[sort_idx]
        pred_motion = pred_motion[sort_idx]
        m_length = m_length[sort_idx]
...
def evaluation_motion_editing_hml(eval_loader, trans, vq_model, writer, ep, eval_wrapper, device, time_steps=10, cond_scale=4, source_cond_scale=2.0, text_delta_scale=1.0, unit_length=4, temperature=1,
    ...
    for batch in tqdm(eval_loader):
        caption, src_motion, tgt_motion, m_length, src_joints, tgt_joints, has_source, task_type, name = batch[:9]
        src_m_length = batch[12] if len(batch) > 12 else m_length   # source len
