...
from VQMotion_trainer import VQMotionTrainer
...
def copy_runtime_training_overrides(saved_cfg, runtime_cfg):
    ...
    for key in (
        "max_epoch", "edit_sample_ratio", "m_drop", "source_cfg_drop_prob", "t_drop",
        "s_drop", "mask_lo", "mask_hi", "mask_schedule", "branch_lr_multiplier",
        "save_high_g2t_checkpoints", "high_g2t_threshold",
    ):
    ...
    for key in ("delta_beta", "use_source_control", "use_vq_delta"):
        if key in runtime_cfg.model:
            saved_cfg.model[key] = runtime_cfg.model[key]
    return saved_cfg
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',
                        type=str,
                        default="./configs/train_vqmotion_hml.yaml",
                        help='config file for VQMotion training')
    ...
    if args.OnGoing_model is not None:
        cfg.exp.is_continue = True
    if args.max_epoch is not None:
        cfg.training.max_epoch = args.max_epoch
    if args.stage != 'source':
        print("stage=text is disabled; keep source-branch stage only")
        args.stage = 'source'
    stage_dirname = getattr(cfg.exp, "stage_dir", "VQMotion")
    ...
    print(f"source control branch: {'enabled' if getattr(cfg.model, 'use_source_control', True) else 'disabled'}")
    print(f"VQ delta branch: {'enabled' if getattr(cfg.model, 'use_vq_delta', False) else 'disabled'}")
    ...
    vqmotion_model = VQMotion(
        cfg=cfg,
        device=device,
        full_length=cfg.data.max_motion_length // cfg.data.unit_length
    )
    if hasattr(vqmotion_model, "set_vq_quantizer") and hasattr(vq_model, "quantizer"):
        vqmotion_model.set_vq_quantizer(vq_model.quantizer)
    elif hasattr(vqmotion_model, "set_vq_codebook") and hasattr(vq_model, "quantizer") and hasattr(vq_model.quantizer, "codebook"):
        vqmotion_model.set_vq_codebook(vq_model.quantizer.codebook)
    pc_vq = sum(param.numel() for param in vqmotion_model.parameters())
    print(f"num of transformer parameter: {pc_vq / 1000} k")

    n_trainable = sum(p.numel() for p in vqmotion_model.parameters() if p.requires_grad)
    print(f"stage: {args.stage}, trainable parameter: {n_trainable / 1000} k")

    trainer = VQMotionTrainer(cfg, vq_model=vq_model, vqmotion_model=vqmotion_model, eval_wrapper=eval_wrapper,
